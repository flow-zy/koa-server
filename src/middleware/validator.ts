...
// 校验是否携带id参数
export async function validatorId(ctx: Context, next: Next) {
  try {
    ctx.verifyParams({
      id: 'number',
    })
...
// 校验上传图片格式
export async function verifyUploadImg(ctx: Context, next: Next) {
  try {
...
// 校验文件
export async function verifyUpload(ctx: Context, next: Next) {
  try {
    const { file } = ctx.request.files!
